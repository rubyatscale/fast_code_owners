...
def gem_install_test(dotgem)
  ...
  testrun = lambda do |cmd|
    cmd = cmd.chomp

    wrapper.call do
      old = ENV.fetch('GEM_HOME', nil)
      ENV['GEM_HOME'] = tmpdir
      ruby "-rfast_code_owners -e '(#{cmd}) || abort'"
      puts "✅ Passed (#{cmd})"
  ...
  testrun.call <<~RUBY
    FastCodeOwners::VERSION == "#{GEMSPEC.version}"
  RUBY
...
namespace :pkg do
  ...
  task ruby: 'pkg' do
    ...
    gemspec_path = 'fast_code_owners.gemspec'
    ...
    Dir.chdir(staging_dir) do
      ...
      Gem::Package.build(final_gemspec, true, false, "#{slug}.gem")
    end
  ...
  %w[x86_64-darwin arm64-darwin x86_64-linux].each do |platform|
