...
RSpec.describe FastCodeOwners do
  it 'has a version number' do
    expect(FastCodeOwners::VERSION).not_to be nil
  end

  describe '.for_file' do
    subject { FastCodeOwners.for_file(file_path) }
  ...
  describe '.for_class' do
    ...
    before do
      allow(FastCodeOwners::FilePathFinder).to receive(:path_from_klass).and_return(file_path)
    end
    ...
    context 'when the klass path is found' do
      ...
      it 'calls for_file with the correct file path' do
        subject
        expect(FastCodeOwners::FilePathFinder).to have_received(:path_from_klass).with(klass)
      end
