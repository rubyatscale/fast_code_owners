RSpec.describe FastCodeOwners::Cli do
  subject { FastCodeOwners::Cli.run!(argv) }
  ...
  describe 'validate' do
    ...
    context 'when run without arguments' do
      it 'runs validations with the right defaults' do
        expect(FastCodeOwners).to receive(:validate!) do |args|
      ...
      context 'without errors' do
        ...
        it 'has empty output' do
          expect(FastCodeOwners::Cli).to_not receive(:puts)
          subject
        end
      end
    end
  ...
  describe 'for_file' do
    ...
    context 'when run with no flags' do
      context 'when run with one file' do
        ...
        it 'outputs the team info in human readable format' do
          expect(FastCodeOwners::Cli).to receive(:puts).with(<<~MSG)
            Team: My Team
      ...
      context 'when run with no files' do
        ...
        it 'outputs the team info in human readable format' do
          expect { subject }.to raise_error 'Please pass in one file. Use `bin/codeowners for_file --help` for more info'
        end
      end
    ...
    context 'when run with --json' do
      ...
      context 'when run with one file' do
        it 'outputs JSONified information to the console' do
          ...
          expect(FastCodeOwners::Cli).to receive(:puts).with(json.to_json)
          subject
        end
      end
  ...
  describe 'using unknown command' do
    ...
    it 'outputs help text' do
      expect(FastCodeOwners::Cli).to receive(:puts).with("'some_command' is not a code_ownership command. See `bin/codeowners help`.")
      subject
    end
  end
  ...
  describe 'passing in no command' do
    ...
    it 'outputs help text' do
      ...
      EXPECTED
      expect(FastCodeOwners::Cli).to receive(:puts).with(expected)
      subject
    end
  end
end
